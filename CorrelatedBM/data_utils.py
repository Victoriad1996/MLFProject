...
data_path = '/Users/victoriadesmarquest/Code/NJODE/data/'
    ...
    if time_id is None:
        ...
        for ll in l:
            if stock_model_name == ll.split('-')[0]:
                models.append(ll)
        print([x for x in models])
        #times = [int(x.split('-')[2]) for x in models]
...
class IrregularMixedDataset(Dataset):
    ...
    def __init__(self, model_name, time_id=None, idx=None):
        ...
        #print(idx)
        #print("len(stock_paths) ", len(stock_paths))
        #print("len(observed_dates) ", len(observed_dates))
        #print("len(nb_obs) ", len(nb_obs))
