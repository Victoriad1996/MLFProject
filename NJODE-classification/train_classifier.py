...
def train(
        ...
        enc_nn=default_enc_nn, use_rnn=True,
        solver="euler", weight=0.5, weight_decay=1.,
        dataset='BlackScholes', dataset_id=None, plot=False, paths_to_plot=(0,),
    ...
    dl = DataLoader(
        dataset=data_train, collate_fn=data_utils.custom_collate_fn_classification,
        shuffle=True, batch_size=batch_size, num_workers=N_DATASET_WORKERS)
    dl_val = DataLoader(
        dataset=data_val, collate_fn=data_utils.custom_collate_fn_classification,
    ...
    params_dict = {
        ...
        'optimal_eval_loss': opt_eval_loss,'n_layers_classification': 3, 
        'activation_classification': 'relu','n_hidden_classification': 50,
        'dropout_rate_classification': 0, 'options': options}
    ...
    if 'other_model' not in options:
        ...
        model_name = 'NJODE'
        #print('other_model not in options')
        #print(params_dict)
...
## 2D dataset
    ...
    #change use_rnn to true in order to use the version implemented in the paper
    train(model_id=None, epochs=100, batch_size=100, save_every=1,
          learning_rate=0.0001, test_size=0.2, seed=398,
          hidden_size=10, bias=True, dropout_rate=0.1,
