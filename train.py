...
default_readout2_nn = ((50,  'tanh'), (50, 'tanh'))
...
def train(
        ...
        ode_nn=default_ode_nn, readout_nn=default_readout_nn, readout2_nn = default_readout2_nn,
    ...
    print(dataset_id)
    print(train_idx)
    print(dataset_metadata["nb_paths"])
    '''
    data_train = data_utils.IrregularMixedDataset(
        model_name=dataset, time_id=dataset_id, idx=train_idx)
    data_val = data_utils.IrregularMixedDataset(
    ...
    if dataset == "BlackScholes_mixed":
    	collate_fn = data_utils.custom_collate_fn_classification
    ...
    params_dict = {
        ...
        'ode_nn': ode_nn, 'readout_nn': readout_nn, 'readout2_nn': readout2_nn, 'enc_nn': enc_nn, 
        'use_rnn': use_rnn,
    ...
    while model.epoch <= epochs:
        ...
        for i, b in tqdm.tqdm(enumerate(dl)):
            ...
            if 'other_model' not in options:
                hT, loss, _ = model(
                    times, time_ptr, X, obs_idx, delta_t, T, start_X, n_obs_ot, labels,
        ...
        with torch.no_grad():
            ...
            for i, b in enumerate(dl_val):
                ...
                if 'other_model' not in options:
                    hT, c_loss, batch_acc = model(
                        times, time_ptr, X, obs_idx, delta_t, T, start_X,
                        n_obs_ot, labels, return_path=False, get_loss=True                
                    )
                elif options['other_model'] == "GRU_ODE_Bayes":
                    M = torch.ones_like(X)
                    hT, c_loss, _, _ = model(
                ...
                num_obs += 1
                accuracy_val += batch_acc

                # mean squared difference evaluation
                if 'evaluate' in options and options['evaluate']:
                    _eval_msd = model.evaluate(
                        times, time_ptr, X, obs_idx, delta_t, T, start_X,
                        n_obs_ot, labels, stockmodel, return_paths=False)
                    eval_msd += _eval_msd

            print("num obs: {}".format(num_obs))
            eval_time = time.time() - t
            loss_val = loss_val / num_obs
            eval_msd = eval_msd / num_obs
            train_loss = loss.detach().numpy()
            accuracy_val = accuracy_val / num_obs
            print("epoch {}, weight={:.5f}, train-loss={:.5f}, "
                  "optimal-eval-loss={:.5f}, eval-loss={:.5f}, accuracy_val={:.5f} ".format(
                model.epoch, model.weight, train_loss, opt_eval_loss, loss_val, accuracy_val))
        if 'evaluate' in options and options['evaluate']:
...
if __name__ == '__main__':
    ...
    # create first a dataset with data_utils.create_mixed_dataset
	...
	'''
	dataset_id = 1621700820
	data = data_utils.IrregularMixedDataset(model_name=dataset, time_id=dataset_id)
	...
	train(model_id=None, epochs=100, batch_size=100, save_every=1,
          ...
          readout_nn=ode_nn, readout2_nn=ode_nn, use_rnn=False,
